...
from lib.analytics import get_dist_from_exec2
...
N_arr = 20
N_exe = 20
exe_dist = get_dist_from_exec2(execute_dur, N_exe, N_arr)
...
plt.plot(x_bg  , y_bg  , linestyle='--', color='blue', drawstyle=cdfsty, clip_on=True, linewidth=2.0)
plt.plot(x_bg2 , y_bg2 , linestyle='--', color='red', drawstyle=cdfsty, clip_on=True, linewidth=2.0)
...
plt.xlim([0, 15])
plt.ylim([0, y_lim])
plt.legend(['M/G(DS)/1 Simu', 'M/G(DS)/1 Average', 'M/G(DS)/1 Worst'])
plt.show()
